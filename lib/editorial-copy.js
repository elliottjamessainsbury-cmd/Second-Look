(function (root, factory) {
  ...
  function firstSentence(text) {
    ...
    const sentence = value.match(/^.{40,180}?[.!?](?:\s|$)/);
    if (sentence) {
  ...
  function withShortText(explanation) {
    ...
    return {
      ...explanation,
      shortText: explanation.shortText || firstSentence(explanation.text),
    };
  }
  ...
  function buildFallbackExplanation({ candidate, scoreData, bestSeed, userProfile }) {
    candidate = {
      ...
      tone: Array.isArray(candidate?.tone) ? candidate.tone : [],
      mood: Array.isArray(candidate?.mood) ? candidate.mood : [],
    };
    scoreData = {
      ...(scoreData || {}),
      directSources: Array.isArray(scoreData?.directSources) ? scoreData.directSources : [],
      themeOverlap: Array.isArray(scoreData?.themeOverlap) ? scoreData.themeOverlap : [],
      toneOverlap: Array.isArray(scoreData?.toneOverlap) ? scoreData.toneOverlap : [],
      moodOverlap: Array.isArray(scoreData?.moodOverlap) ? scoreData.moodOverlap : [],
      paceOverlap: Array.isArray(scoreData?.paceOverlap) ? scoreData.paceOverlap : [],
    };
    ...
    const broadSignal = candidate.themes[0] || candidate.tone[0] || candidate.mood[0] || candidate.director || candidate.title;
    return {
  ...
  function explanationForCandidate({ candidate, scoreData, bestSeed, lookups, userProfile }) {
    ...
    return withShortText(buildFallbackExplanation({ candidate, scoreData, bestSeed, userProfile }));
  }
