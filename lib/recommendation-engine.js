(function (root, factory) {
  ...
  function questionnaireToSignals(answers) {
    ...
    // Every emitted mood/theme/pace value MUST belong to the controlled
    // vocabulary in scripts/build_film_taste_profiles.py — those are the only
    // fields the deterministic (no-embedding) scorer overlaps against. Values
    // outside it (e.g. "medium"/"fast" paces, "formal"/"patient" moods) never
    // match a candidate, so the answer would rank identically to unanswered.

    // Black & white
    if (answers.bw === "timeless") {
      preferredEras.push("pre-1970", "1970s");
      mood.push("contemplative");
    } else if (answers.bw === "depends") {
      preferredEras.push("1970s");
    } else if (answers.bw === "homework") {
      lowWeightMood.push("playful", "exhilarating");
    }

    // Slow films / pace
    if (answers.slow === "hypnotic") {
      mood.push("contemplative", "melancholic");
      preferredPaces.push("slow");
    } else if (answers.slow === "depends") {
      preferredPaces.push("measured");
    } else if (answers.slow === "move_it") {
      preferredPaces.push("brisk");
      lowWeightMood.push("exhilarating");
    }

    // Ambiguous endings
    if (answers.ambiguity === "love") {
      themes.push("memory");
      mood.push("dreamlike", "eerie");
    } else if (answers.ambiguity === "sometimes") {
      themes.push("memory");
    } else if (answers.ambiguity === "clear") {
      lowWeightMood.push("hopeful");
    }

    // Ari Aster's worst film — a low-weight taste probe
    if (answers.ari_aster === "hereditary") {
      lowWeightMood.push("dreamlike", "playful");
    } else if (answers.ari_aster === "midsommar") {
      lowWeightMood.push("unsettling");
    ...
    // Legacy calibration questions (not currently shown, retained for
    // compatibility; kept in the controlled vocabulary so they score if re-added)
    if (answers.weird === "max") {
      mood.push("dreamlike", "unsettling");
      themes.push("obsession", "identity");
    } else if (answers.weird === "medium") {
      mood.push("eerie");
      themes.push("displacement");
    }
